...
class Venue(db.Model):
    ...
    # I implemented these below
    #################################
    website = db.Column(db.String(500))
    genres = db.Column(db.String(120))
    searching_talent = db.Column(db.Boolean, default=False)
    description = db.Column(db.String(500), default=False)
    #################################
...
class Artist(db.Model):
    ...
    venues = db.relationship('Venue', secondary='shows', backref=db.backref('backref_artist', lazy=True))

# TODO Implement Show and Artist models, and complete all model relationships and properties, as a database migration.
shows = db.Table('shows',
    db.Column('artist_id', db.Integer, db.ForeignKey('artists.id'), primary_key=True),
    db.Column('venue_id', db.Integer, db.ForeignKey('venues.id'), primary_key=True),
    db.Column('time', db.String(120))
)
#----------------------------------------------------------------------------#
# Filters.
#----------------------------------------------------------------------------#
        ...
        if form.validate_on_submit():
            try:
                ...
                new_venue = Venue(
                    ...
                    genres=genres, searching_talent=seeking_talent, description=seeking_description 
                )
            # import pdb
            # pdb.set_trace()
    ...
    if form.validate_on_submit():
        try:
            ...
            new_artist = Artist(
                ...
                image_link=image_link, facebook_link=facebook_link, website=website_link, 
                genres=genres, searching_venue=seeking_venue, description=seeking_description 
            )
...
def shows():
  ...
  import pdb
  pdb.set_trace()
  data = Artist.query(shows).join(Artist).all()
